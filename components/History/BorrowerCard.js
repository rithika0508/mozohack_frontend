...
const BorrowerCard=(props)=>{
    ...
    return(
        ...
        <div style={{display:"flex",justifyContent:'space-evenly',alignItems:"center",backgroundColor:"#E2E2E2",width:"73%",paddingTop:"1rem",borderRadius:"0.2rem"}}>
            ...
            <div>
                <button style={{marginBottom:"3.9rem",backgroundColor:"#366FD1",color:"white",fontWeight:"bold",border:"none",width:"6rem",height:"2rem",borderRadius:"0.5rem",marginLeft:"8rem"}} onClick={onOpenModal}>Repay</button>
                <p style={{fontFamily:"sans-serif",fontWeight:"bold",fontSize:"20px"}}>Amount to be Paid: {amountToBePaid}</p>
            </div>
        </div>
        <RepayModal setForceUpdate={props.setForceUpdate} open={open} onCloseModal={onCloseModal} requestId={props?.data?.requestId} setAmountToBePaid={setAmountToBePaid} amountToBePaid={amountToBePaid}/>
        </div>
    )
}
